...
#
# NOTE: a single worker is intentional. The API keeps the detector baseline,
# rolling statistics, rate-limiter buckets, and WebSocket connections in
# process-local memory. Running multiple workers would give each its own
# divergent baseline, so /score, /stats, and /health would return
# inconsistent results depending on which worker handled the request, and
# rate limiting would be enforced per-worker rather than globally. To scale
# out horizontally, move that shared state into an external store (e.g. Redis)
# first, then raise the worker/replica count.
